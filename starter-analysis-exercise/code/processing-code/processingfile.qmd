...
MJ: rename `Daily Exercise minutes` for simplicity in analysis. This variable should also be numeric but it was loaded as character.

```{r}
d5 <- d4 %>% dplyr::rename(Exercise = `Daily Exercise minutes`)
d5$Exercise <- as.numeric(d5$Exercise)
```

MJ: rename `Star sign` to avoid spaces in the analysis. This variable should be categorical/factor but it was loaded as character.

```{r}
d6 <- d5 %>% dplyr::rename(Sign = `Star sign`)
d6$Sign <- as.factor(d6$Sign)
